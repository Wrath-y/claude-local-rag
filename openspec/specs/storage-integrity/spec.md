...
服务 SHALL 维护 `storage/manifest.json` 文件，记录最后一次成功 commit 的一致性摘要，字段包括 `version`、`committed_at`、`chunks.path`、`chunks.count`、`chunks.sha256`、`index.path`、`index.dim`、`index.ntotal`、`index.sha256`。每次写操作在 chunks 与 index 都成功原子替换后 SHALL 原子地更新 manifest；chunks 或 index 任一替换失败 MUST NOT 更新 manifest。
